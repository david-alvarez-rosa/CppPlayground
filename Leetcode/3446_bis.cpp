...
class Solution {
 public:
  [[nodiscard]] static auto sortMatrix(Matrix& grid) noexcept -> Matrix {
    ...
    for (auto& [_, bottom_diag] : bottom_diags) {
      std::sort(bottom_diag.begin(), bottom_diag.end(), std::less{});
    }

    for (auto& [_, top_diag] : top_diags) {
      std::sort(top_diag.begin(), top_diag.end(), std::greater{});
    }
