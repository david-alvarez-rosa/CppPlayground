...
class Solution {
 public:
  ...
  static auto isValidSudoku(const Board& board) noexcept -> bool {
    ...
    auto rows = std::vector<std::vector<bool>>(n, std::vector<bool>(n, false));
    ...
    auto cols = std::vector<std::vector<bool>>(n, std::vector<bool>(n, false));
    for (auto col_i = 0ZU; col_i < n; ++col_i) {
      for (auto row_i = 0ZU; row_i < n; ++row_i) {
        auto cell = board[row_i][col_i];
        if ('.' == cell) {
    ...
    auto boxes = std::vector<std::vector<bool>>(n, std::vector<bool>(n, false));
    auto box_id = 0ZU;
