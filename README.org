...
generator); ~clang-format~ (formatter); ~lldb~ (or ~gdb~) for debugging;
and ~llvm-cxxfilt~ (or ~c++filt~) to demangle the assembly output.
...
#+begin_src sh
  ...
  $ sudo dnf install clang clang-tools-extra lldb llvm cmake ninja-build ccache mold        # Fedora
#+end_src
