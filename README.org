...
Library dependencies (GoogleTest, GoogleBenchmark) are managed with
Conan
...
#+begin_src sh
  ...
  $ conan install . -of build/debug -pr conan/profile --build=missing --lockfile=conan.lock -s build_type=Debug  # Debug
#+end_src
...
Edit ~scratch/assembly.cpp~ file (use the ~debug~ preset instead,
