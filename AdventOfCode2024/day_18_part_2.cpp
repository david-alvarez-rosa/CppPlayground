...
class MemorySpace final {
 public:
  explicit MemorySpace(std::vector<Pos> byte_positions,
...
auto main(int argc, char* argv[]) -> int {
  ...
  for (auto i{1024};; ++i) {
    auto memory_space = MemorySpace{positions, i};
    if (-1 == memory_space.CountSteps()) {  // I know, I know... this sucks
      std::cout << positions[i - 1].col << ',' << positions[i - 1].row << "\n";
      return 0;
    }
  }
}
