...
auto CompactDisk(Disk& disk) {
  ...
  while (right >= 0) {
    // Next block
    while (right >= 0 && disk[right] == kFreeBlockId) {
      --right;
    }
    auto block_id = disk[right];
    auto block_size{0};
    ...
    // Free block
    auto free_block_found{false};
    auto left{0};
    while (!free_block_found && left < right) {
      if (disk[left] != kFreeBlockId) {
        ++left;
        continue;
      }
      auto free_block_size{0};
      while (disk[left] == kFreeBlockId && free_block_size < block_size) {
        ++left;
        ++free_block_size;
      };
    ...
    if (free_block_found) {
      while (block_size--) {
        disk[right + 1 + block_size] = kFreeBlockId;
        disk[left - 1 - block_size] = block_id;
      }
    }
  }
}
