...
class RingBufferV5 {
  ...
  alignas(std::hardware_destructive_interference_size) std::atomic_size_t head_{
      0};
  alignas(std::hardware_destructive_interference_size) std::size_t head_cached_{
      0};
  alignas(std::hardware_destructive_interference_size) std::atomic_size_t tail_{
      0};
  alignas(std::hardware_destructive_interference_size) std::size_t tail_cached_{
      0};
...
static auto BM_RingBuffer(benchmark::State& state) -> void {
...
BENCHMARK(BM_RingBuffer<RingBufferV2<int, 100'000>>)->Arg(100'000'000);
BENCHMARK(BM_RingBuffer<RingBufferV3<int, 100'000>>)->Arg(100'000'000);
BENCHMARK(BM_RingBuffer<RingBufferV4<int, 100'000>>)->Arg(100'000'000);
BENCHMARK(BM_RingBuffer<RingBufferV5<int, 100'000>>)->Arg(100'000'000);
